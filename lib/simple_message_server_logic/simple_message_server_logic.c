    ...
    )
{
        ...
        if (fstat(i, &statbuf) == 0)
	{
            (void) fprintf(
                stderr,
                "%s: %s: some files above STDERR_FILENO "
                "are still open.\n",
		cmd,
		__func__
                );
    ...
    )
{
    ...
    else if ((size_t) cnt >= url_len)
    {
        (void) fprintf(
            stderr,
            "%s: %s: snprintf() failed - buffer too small.\n",
	    cmd,
	    __func__
            );
        exit(EXIT_FAILURE);
    ...
    if (strlen(pw->pw_dir) >= homedir_len)
    {
        (void) fprintf(
            stderr,
            "%s: %s: buffer for homedir too small.\n",
	    cmd,
	    __func__
            );
        exit(EXIT_FAILURE);
    ...
    )
{
    ...
    if (buf[len] != 0)
    {
        (void) fprintf(
            stderr,
            "%s: %s: fatal error. input is not null-terminated.\n",
	    cmd,
	    __func__
            );
        exit(EXIT_FAILURE);
